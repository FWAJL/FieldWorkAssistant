...
$target_path = "uploadfiles/upload_location/";
...
if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
    ...
    alert("The file <?php echo $_FILES['uploadedfile']['name']; ?> was uploaded.  Locations were added to <?php if(isset($row4['project_name'])) { echo $row4['project_name'].""; } ?>");
  </script>
...
$query_Recordset1 = "SELECT * FROM location_info";
...
REPLACE INTO TABLE location_info 
FIELDS TERMINATED BY "," 
 (loc_name, l_lat, l_long)
;')
...
//<!-- Last inserted -->
 ...
 $sql1=mysql_query("SELECT * FROM  `location_info` WHERE  `loc_id` = (SELECT MAX(  `loc_id` ) FROM  `location_info` )");
...
mysql_select_db($database_New_db, $New_db);
$query_Recordset2 = "SELECT * FROM location_info";
...
$num_rows=$totalRows_Recordset2-$totalRows_Recordset1;
  ...
  for ($x=$row1['loc_id']-$num_rows+1; $x<=$row1['loc_id']; $x++)
   {

 $in=mysql_query("INSERT INTO location_to_project SET loc_id='$x',project_id='".$proj_id."'");
