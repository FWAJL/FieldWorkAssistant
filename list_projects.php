...
<?php 
	...
	include('config.php');
	$query1 = mysql_query("SELECT * FROM project_info");
...
/*$prj=mysql_query("select project_id from project_info  LIMIT {$startpoint} , {$limit} "); */
...
if(isset($_REQUEST['save'])){
	...
	if(isset($_REQUEST['cbox'])) {
		...
		mysql_query("UPDATE project_info SET active_bool=1 WHERE project_id IN (".implode(',', $checked).")");
		mysql_query("UPDATE project_info SET active_bool=0 WHERE project_id IN (".implode(',', $unchecked).")");
	} else {
...
<link href="css/pagination.css" rel="stylesheet" type="text/css" />
	...
	<link href="paging.css" rel="stylesheet" type="text/css" />
				 ...
				 <input type="hidden" name="list_projects" value="1">
		     ...
		     <table>
				 ...
				 <td width="45%">
				    <div style="width:400px;"><h3 id="com_name">
                    Project Name</h3></div>
				  </td>
 ...
 <?php 
//This mahy be useless code
$k=0;
$p=0;
$arr=array();
$y=ceil($x/$limit);
$query = mysql_query("SELECT * FROM project_info LIMIT {$startpoint}
...
<input type="checkbox" name="cbox[<?php  echo $row['project_id']; ?>]" value="0">
<input type="hidden" name="id[<?php  echo $row['project_id']; ?>]" value="<?php  echo $row['project_id']; ?>">
				  <?php 
	}
	...
	if($row['active_bool']==1) {
		?>                 
<input type="checkbox" checked="checked" name="cbox[<?php  echo $row['project_id']; ?>]" value="1">
<input type="hidden" name="id[<?php  echo $row['project_id']; ?>]" value="<?php  echo $row['project_id']; ?>">
				 <?php  } ?>
				 </td>
				<td width="45%" height="25">
                <h3 class="del_com">
				<a href="index.php?pid=<?php  echo $row['project_id']; ?>&amp;show_project=1"><?php  echo $row['project_name']; ?></a>
				</h3>
				 </td>
         <td width="50%" align="left" style="padding-left:30px">         
				<?php  if ($row['visible']==1) { ?>
			<a href="index.php?list_projects=1&amp;pid=<?php  echo $row['project_id']; ?>&amp;visible=<?php  echo $row['visible']; ?>&amp;vis_pro=1&amp;page=<?php 
		...
		?>&amp;perpage=<?php  echo $limit; ?>"><img src="images/rightcheck.png"  width="15" height="15"/> </a>
			<?php
			 ...
			 <a href="index.php?list_projects=1&amp;visible=<?php  echo $row['visible']; ?>&amp;vis_pro=1&amp;page=<?php 
		
		if(isset($_REQUEST['page'])){
			echo $_REQUEST['page'];
		}

		?>&amp;pid=<?php  echo $row['project_id']; ?>&amp;&amp;perpage=<?php  echo $limit; ?>"><img src="images/crosscheck.png" width="15" height="15"/> </a>
			 <?php
 } ?>
				</td>
				<td></td>
...
<?php  if($page>1) { ?><li class="next"><a href="?page=<?php  echo $page-1; ?>
												&amp;perpage=<?php 
		...
		?>
                                                ...
                                                Previous</a></li> <?php  } ?>
<?php  for($i=1;$i<=$y;$i++){ ?>
<a href="?page=<?php  echo $i; ?>&amp;list_projects=1
                                &amp;perpage=<?php 
...
<input name="list_projects" type="hidden" value="1" />
