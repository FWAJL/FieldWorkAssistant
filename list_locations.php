...
$(document).ready(function(){
	$("#loc_frm").submit(function(){
		if ($("#loc_grp_name").val() != '') {
		return confirm("This will create a group: " + $("#loc_grp_name").val() + ".  Are you sure?");
		}
	});
});
</script>     
...
$query1 = mysql_query("SELECT * FROM location_to_project WHERE project_id='".$_REQUEST['pid']."'");
...
/*$loc=mysql_query("select loc_id from location_info  LIMIT {$startpoint}
...
$dup = mysql_query("SELECT TRUE FROM loc_groupname_to_group WHERE loc_group_name='".$_REQUEST['loc_grp_name']."' LIMIT 1");
...
$sql_new_group=mysql_query("INSERT INTO loc_groupname_to_group SET  
	 loc_group_name='".$_REQUEST['loc_grp_name']."'")
	 or die(mysql_error());

	  $lastinserted=mysql_insert_id();
	  
$sql_group_to_project=mysql_query("INSERT INTO loc_group_to_project SET  
	 loc_group_id='".$lastinserted."',
...
if(isset($_REQUEST['cbox'])) 
		...
		foreach($_REQUEST['id'] as $key => $id) 
			...
			mysql_query("INSERT INTO location_to_group SET
			 loc_id='".$key."',
			 loc_group_id='".$lastinserted."'") or die ("Error in query: $sql"); 
						}
					}
				}
			}
		
echo "<meta http-equiv='refresh' content='0;url=index.php?list_locations=1&amp;pid=".$_REQUEST['pid']."'>";
...
<input type="hidden" name="list_locations" value="1" />
...
<input type="hidden" name="id[<?php  echo $row['loc_id']; ?>]" value="<?php  echo $row['loc_id']; ?>">
				...
				<td height="25">
                <h3 class="del_com">
				<a href="index.php?pid=<?php echo $_REQUEST['pid']; ?>&amp;lid=<?php  echo $row['loc_id']; ?>&amp;edit_location=1"><?php  echo $row['loc_name']; ?></a>
				</h3>
				 </td>
         <td align="left" style="padding-left:30px">         
				<?php  if ($row['visible']==1) { ?>
			<a href="index.php?pid=<?php echo $_REQUEST['pid']; ?>&amp;list_locations=1&amp;lid=<?php  echo $row['loc_id']; ?>&amp;visible=<?php  echo $row['visible']; ?>&amp;vis_loc=1&amp;page=<?php 
		...
		?>&amp;lid=<?php  echo $row['loc_id']; ?>&amp;&amp;perpage=<?php  echo $limit; ?>"><img src="images/crosscheck.png" width="15" height="15"/> </a>
			 <?php
 } ?>
				</td>
...
<td width="281"> 
<input value="" name="loc_grp_name" type="text" id="loc_grp_name" style="font-size:9px" size="40" />
</td>
</tr>
...
<td></td>
<td></td>
<td><label for="loc_grp_name">Enter a name here to save as a group.</label>
                 </td>
              </tr>
			 </table> 
				</form>
...
<?php  if($page>1) { ?><li class="next"><a href="?pid=<?php echo $_REQUEST['pid']; ?>&amp;page=<?php  echo $page-1; ?>
		...
		?>
                                                ...
                                                Previous</a></li> <?php  } ?>
<?php  for($i=1;$i<=$y;$i++){ ?>
<a href="?page=<?php  echo $i; ?>&amp;list_locations=1
...
<?php  if($page<$y) { ?><li class="next"><a href="?page=<?php  echo $page+1; ?>
		...
		?>
										          &amp;list_locations=1">Next </a></li> <?php  } ?>
</ul> 
<?php  } ?>
<br />
<br />
...
<input name="list_locations" type="hidden" value="1" />
 <input type="hidden" name="pid" value="<?php echo $_REQUEST['pid'];?>" />
